...
df_MarketCap = pd.DataFrame() # Initialize the variable
...
def lookup(sym):
    ...
    if df_MarketCap.empty:
        df_MarketCap = read_csv(DIR / "data" / "marketcap_data.csv")
    
    filtered_df = df_MarketCap[df_MarketCap['Symbol'] == sym.upper()]
    ...
    try:
        df["TO_percent"] = ((df["Volume"] * df["Close"])/(filtered_df["MarketCap"].iloc[0]*10000000))*100
    except Exception as e:   
        print(f"lookup {sym} An error occurred: {e}")
...
if args.lookup:
    ...
    # Heading text
    heading += (
        f'{c.WHITE}SCRIP{" " * 8}DQ{" " * 9}TQ{" " * 9}VOL{" " * 5}IM{c.ENDC}"TO_percent"\n'
    )
    ...
    for sym in symList:
        ...
        if args.date:
            ...
            df = df.loc[df.index < filter_date]
        ...
        open = df.loc[df.index[-1], 'Open']

        if(open>close):     # only interested in the uptrend stocks
            continue
        ...
        tradeQty, dlvQty, volume, close = df.loc[
            df.index[-1], ["QTY_PER_TRADE", "DLV_QTY", "Volume","Close"]
        ]
        ...
        status = "$$" if dq > 1.2 and tq > 1.2 else ""
        try:
            TO_percent = ((volume * close)/(filtered_df["MarketCap"].iloc[0]*10000000))*100
        except Exception as e:   
            print(f"{sym} - An error occurred: {e}")
            continue
        ...
        my_dictionary = {"SCRIP": sym[:15].upper(), "DQ": dq, "TQ": tq, "VOL": vol, "IM": status, "TO_percent": TO_percent}        
